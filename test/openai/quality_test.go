...
func TestRequestBudgetStopsRetriesAndIncludesSummary(t *testing.T) {
	...
	provider.SetHistory([]responses.ResponseInputItemUnionParam{
		responses.ResponseInputItemParamOfMessage(oldText, responses.EasyInputMessageRoleUser),
		responses.ResponseInputItemParamOfMessage("old answer", responses.EasyInputMessageRoleAssistant),
		responses.ResponseInputItemParamOfMessage("recent", responses.EasyInputMessageRoleUser),
	})
	var partial bool
...
func TestSummaryRequestIsObserved(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { writeCompletedText(t, w, "done") }))
	defer server.Close()
	provider := testStreamingProvider(t, server, "instructions")
