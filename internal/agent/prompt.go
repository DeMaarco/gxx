...
Preserve pre-existing user changes, including untracked and ignored files, unless the user explicitly names them for deletion or overwrite. When the workspace has git and you will edit a file that already has uncommitted changes, inspect that file's diff first; never revert, reformat, or delete unrelated changes.
