...
func TestParseRateLimitReadsRemainingQuota(t *testing.T) {
	...
	if parseRateLimit(http.Header{}).Known {
		t.Fatal("empty headers should be unknown")
	}
}

func TestParseAPIErrorAcceptsStringOrObject(t *testing.T) {
	if got := parseAPIError([]byte(`{"error":"You have insufficient permissions for this operation."}`)); got != "You have insufficient permissions for this operation." {
		t.Fatalf("string error = %q", got)
	}
	if got := parseAPIError([]byte(`{"error":{"message":"forbidden","type":"invalid_request_error"}}`)); got != "forbidden" {
...
func TestFetchAccountUsageSumsSpendAndRemainingQuota(t *testing.T) {
	...
	account := fetchAccountUsage(context.Background(), server.Client(), server.URL, "test-key", now)
	if account.Error != "" {
...
func TestFetchAccountUsageExplainsMissingAdminAccess(t *testing.T) {
	...
	account := fetchAccountUsage(context.Background(), server.Client(), server.URL, "test-key", time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC))
