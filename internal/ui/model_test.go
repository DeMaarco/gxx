...
func TestParseModelCommand(t *testing.T) {
	command, err := parseModelCommand("/model")
	if err != nil || !command.Show {
		t.Fatalf("parse /model = %+v, %v, want Show", command, err)
	}

	command, err = parseModelCommand("/model terra context=1m effort=high fast=on")
	if err != nil {
		t.Fatal(err)
	}
	...
	command, err = parseModelCommand("/model context 128k fast off")
	if err != nil {
		t.Fatal(err)
	}
